...
impl Transport {
    ...
    pub fn default_headers(api_key: Option<&str>) -> header::HeaderMap<header::HeaderValue> {
        ...
        headers.insert(
            header::USER_AGENT,
            header::HeaderValue::from_static("les-bot"),
        );
        headers.insert(
            header::CONTENT_TYPE,
    ...
    fn check_key(&self) -> Result<(&str, &str)> {
        match self.credential.as_ref() {
            None => Err(LessError::NoApiKeySet()),
            Some((k, s)) => Ok((k, s)),
        }
    }
        ...
        O: DeserializeOwned,
    {
        match response.status() {
            ...
            StatusCode::INTERNAL_SERVER_ERROR => Err(LessError::InternalServerError()),
            StatusCode::SERVICE_UNAVAILABLE => Err(LessError::ServiceUnavailable()),
            StatusCode::UNAUTHORIZED => Err(LessError::Unauthorized()),
            StatusCode::BAD_REQUEST => {
                let error: BinanceContentError = response.json().await?;

                Err(LessError::Generic(Box::new(error)))
            }
            s => Err(LessError::UnkownResponse(format!(
                "Received response: {:?}",
                s
            ))),
        }
