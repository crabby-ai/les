//! This module provides functionality for communicating with the less-binance API.
...
use model::{
    KlineSummaries,
    MarketPair as BinanceMarketPair, ORDER_TYPE_LIMIT, ORDER_TYPE_MARKET, SymbolFilter,
...
impl ExchangeAccount for Binance {
    ...
    async fn cancel_order(&self, req: &CancelOrderRequest) -> Result<OrderCanceled> {
        if let Some(pair) = req.market_pair.as_ref() {
            ...
            Err(LessError::MissingParameter(
    ...
    async fn get_order(&self, req: &GetOrderRequest) -> Result<Order> {
        let pair = req.market_pair.clone().ok_or_else(|| {
            LessError::MissingParameter("market_pair parameter is required.".to_string())
        })?;
        let u64_id = req
            .id
            .parse::<u64>()
    ...
    fn try_from(req: &GetOrderHistoryRequest) -> Result<Self> {
        Ok(Self {
            ...
            symbol: req.market_pair
                .clone()
                .map(|market| BinanceMarketPair::from(market).0)
                .ok_or_else(|| LessError::MissingParameter("market_pair parameter is required.".to_string()))?,
        })
    }
}
